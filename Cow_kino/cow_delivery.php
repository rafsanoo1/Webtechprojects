...
if (!isset($_COOKIE['cowkino_client_id'])) {
  $newId = bin2hex(random_bytes(16)); // 32 chars
  setcookie('cowkino_client_id', $newId, time() + (86400 * 365), "/"); // 1 year
  $_COOKIE['cowkino_client_id'] = $newId; // available immediately
}
$clientId = $_COOKIE['cowkino_client_id'];

// 2) Fetch delivery history for THIS client only
$history = [];
$stmt = $conn->prepare("SELECT name, email, contact, pickup_location, dropoff_location, distance_km, cow_count, delivery_charge, created_at
                        FROM cow_delivery
                        WHERE client_id = ?
                        ORDER BY id DESC");
...
<head>
  ...
  <style>
    ...
    :root{
      ...
      /* TEXT */
      --text: #0f172a;        /* dark premium */
      --muted: rgba(15,23,42,0.65);

      /* GREEN THEME */
      --accent: #16a34a;      /* premium green */
...
<body>
  ...
  <section class="market">
    <div class="container">
      ...
      <div class="cow-card">
        ...
        <form id="deliveryForm" action="submit_delivery.php" method="POST" autocomplete="on">
          <div class="grid">
            <div>
  ...
  <script>
    ...
    function calcCharge(){
      ...
      if (!isFinite(d) || !isFinite(c) || d <= 0 || c <= 0){
        chargeEl.value = "0";
        return;
      }
      const charge = d * 120 * (c * .75);
